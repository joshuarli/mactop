...
final class CPUProcessUsageDeltaTests: XCTestCase {
    func testNormalDelta() {
        let pct = calculateCPUProcessUsagePercent(current: (time: 2_000_000_000, start: 1),
    ...
    func testHalfCore() {
        let pct = calculateCPUProcessUsagePercent(current: (time: 500_000_000, start: 1),
    ...
    func testMultiThreadedExceeds100() {
        let pct = calculateCPUProcessUsagePercent(current: (time: 4_000_000_000, start: 1),
                           previous: (time: 0, start: 1),
                           elapsed: 1.0)
    ...
    func testPIDReuseReturnedNil() {
        let pct = calculateCPUProcessUsagePercent(current: (time: 1_000_000_000, start: 200),
    ...
    func testTimeWentBackwardsReturnsNil() {
        let pct = calculateCPUProcessUsagePercent(current: (time: 100, start: 1),
    ...
    func testElapsedZeroReturnsNil() {
        let pct = calculateCPUProcessUsagePercent(current: (time: 1_000_000_000, start: 1),
                           previous: (time: 0, start: 1),
                           elapsed: 0)
        XCTAssertNil(pct)
    }
    ...
    func testScalesWithElapsed() {
        ...
        let pct1 = calculateCPUProcessUsagePercent(current: (time: 1_000_000_000, start: 1), previous: (time: 0, start: 1), elapsed: 1.0)
        let pct2 = calculateCPUProcessUsagePercent(current: (time: 1_000_000_000, start: 1), previous: (time: 0, start: 1), elapsed: 2.0)
