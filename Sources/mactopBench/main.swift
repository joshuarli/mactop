...
private struct MactopBench {
  ...
  static func main() {
    ...
    for child in children {
      ...
      guard child.process.terminationStatus == 0,
        ...
        let result = try? JSONDecoder().decode(BenchResult.self, from: data)
      else {
        writeError(
          "benchmark child failed with status \(child.process.terminationStatus)", status: 1)
      }
    ...
    for subsystem in ["power", "gpu"] {
      ...
      for phase in result.phases {
        ...
        print(
          "  \(phase.name.padding(toLength: 27, withPad: " ", startingAt: 0)) \(MactopFormat.string( "%5d", phase.count)) \(MactopFormat.string( "%8.2f", wallMilliseconds)) \(MactopFormat.string( "%12.3f", perTick))"
        )
      }
    }
  }
  ...
  private static func measureSubsystem(named name: String, configuration: BenchConfiguration)
    -> BenchResult
  {
    switch name {
    case "cpu":
    ...
    case "gpu":
      ...
      let reader = GPUUsageReader(
        updateInterval: configuration.interval, phaseRecorder: phaseRecorder)
      return measure(
    ...
    case "power":
      let phaseRecorder = CoreReadPhaseRecorder()
      let reader = PowerTelemetryReader(
        updateInterval: configuration.interval, phaseRecorder: phaseRecorder)
      return measure(
        name: name, configuration: configuration,
        sample: {
  ...
  private static func formatBytes(_ bytes: UInt64, width: Int) -> String {
    ...
    switch bytes {
    case 1_000_000_000...:
      value = MactopFormat.string("%.1fG", Double(bytes) / 1_000_000_000)
    case 1_000_000...:
      value = MactopFormat.string("%.1fM", Double(bytes) / 1_000_000)
    case 1_000...:
      value = MactopFormat.string("%.1fK", Double(bytes) / 1_000)
    default:
      value = "\(bytes)"
    }
  ...
  private static func formatSeconds(_ seconds: TimeInterval) -> String {
    MactopFormat.string("%.2f", seconds)
  }
}
