...
func smoothMetricValue(
  _ value: Double, previous: Double?, alpha: Double = metricGraphSmoothingAlpha
) -> Double {
...
struct PairHistory {
  ...
  init(capacity: Int) {
    let capacity = max(capacity, 1)
    values = Array(
      repeating: MetricHistoryPoint(date: .distantPast, value: (up: 0, down: 0)), count: capacity)
  }
  ...
  mutating func removeAll() {
    values = Array(
      repeating: MetricHistoryPoint(date: .distantPast, value: (up: 0, down: 0)),
      count: values.count)
    nextIndex = 0
    count = 0
    smoothedUp = nil
