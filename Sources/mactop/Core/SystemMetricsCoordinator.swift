...
public final class SystemMetricsCoordinator {
    ...
    private let coordinatorQueue = DispatchQueue(label: "mactop.system-metrics-coordinator", qos: .utility)
    ...
    private func refreshAll(onCPU: @escaping (CPUUsageDetail) -> Void,
        ...
        if !networkReadInFlight {
            ...
            networkQueue.async { [weak self] in
                guard let self else { return }
                let network = self.networkReader.readNetworkUsageDetail(includeHistory: includeNetworkHistory)
                self.coordinatorQueue.async {
