...
@discardableResult
private func addPopupValueRow(_ view: NSView, title: String, value: String) -> (
  ...
  let lbl = LabelField(
    title, frame: NSRect(x: 0, y: (rowHeight - 16) / 2, width: labelWidth, height: 16))
  let val = ValueField(
    value, frame: NSRect(x: labelWidth, y: (rowHeight - 16) / 2, width: w - labelWidth, height: 16))
...
@discardableResult
private func addPopupColorValueRow(_ view: NSView, color: NSColor, title: String, value: String)
  -> (NSView, LabelField, ValueField)
{
  ...
  let lbl = LabelField(
    title, frame: NSRect(x: 18, y: (rowHeight - 16) / 2, width: labelWidth, height: 16))
  let val = ValueField(
    value,
    frame: NSRect(
      x: 18 + labelWidth, y: (rowHeight - 16) / 2, width: w - labelWidth - 18, height: 16))
...
private final class RankedProcessListView: NSView {
  ...
  init(frame: NSRect, count: Int, valueHeader _: String) {
    ...
    for i in 0..<count {
      ...
      let iconView = NSImageView(
        frame: NSRect(x: 2, y: (rowHeight - 16) / 2, width: 16, height: 16))
      iconView.imageScaling = .scaleProportionallyDown

      let nameLabel = LabelField(
        "", frame: NSRect(x: 20, y: (rowHeight - 16) / 2, width: nameW - 20, height: 16))
      nameLabel.cell?.lineBreakMode = .byTruncatingTail

      let valueLabel = ValueField(
        "", frame: NSRect(x: nameW, y: (rowHeight - 16) / 2, width: valueW, height: 16))
  ...
  private func iconForProcess(_ process: RankedProcessMetric) -> NSImage? {
    ...
    let icon = NSWorkspace.shared.runningApplications
      .first {
        $0.localizedName == process.name || $0.executableURL?.lastPathComponent == process.name
      }
      .flatMap { $0.icon }
...
private func formatNetworkSpeedValue(_ bytes: Double) -> (String, String) {
  ...
  case 1_000..<(1_000 * 1_000): return (MactopFormat.string("%.0f", kb), "KB/s")
  case 1_000..<(1_000 * 1_000 * 100): return (MactopFormat.string("%.1f", mb), "MB/s")
  case (1_000 * 1_000 * 100)..<(1_000 * 1_000 * 1_000):
    return (MactopFormat.string("%.0f", mb), "MB/s")
  default: return (MactopFormat.string("%.1f", gb), "GB/s")
  }
}
...
final class CPUPopupView: NSStackView {
  ...
  private func initDashboard() -> NSView {
    ...
    let usageView = NSView(
      frame: NSRect(x: usageX, y: (dashH - usageSize) / 2, width: usageSize, height: usageSize))

    circle = MetricPieChartView(
      frame: NSRect(x: 0, y: 0, width: usageSize, height: usageSize),
  ...
  private func initChart() -> NSView {
    ...
    lineChart = MetricLineChartView(
      frame: NSRect(x: 1, y: 0, width: popupWidth - 2, height: chartContentH),
      num: initialChartSamples, fixedMax: 1)
    ...
    columnChart = CoreUsageColumnChartView(
      frame: NSRect(
        x: spacing, y: spacing, width: popupWidth - spacing * 2, height: barContentH - spacing * 2),
  ...
  private func initDetails() -> NSView {
    ...
    let container = NSStackView(
      frame: NSRect(x: 0, y: 0, width: popupWidth, height: sep.frame.origin.y))
    container.orientation = .vertical
    container.spacing = 0
    view.addSubview(container)

    systemField =
      addPopupColorValueRow(container, color: systemColor, title: "System:", value: "").2
    userField = addPopupColorValueRow(container, color: userColor, title: "User:", value: "").2
    idleField =
      addPopupColorValueRow(
        container, color: idleColor.withAlphaComponent(0.5), title: "Idle:", value: ""
      ).2
  ...
  func updateCPUUsage(
    ...
    processesView.setRankedProcesses(processes) { v in
      MactopFormat.string(v < 10 ? "%.1f%%" : "%.0f%%", v)
    }
  }
...
final class RAMPopupView: NSStackView {
  ...
  private func initDashboard() -> NSView {
    ...
    circle = MetricPieChartView(
      frame: NSRect(x: circleX, y: 0, width: circleSize, height: circleSize),
      segments: [], drawValue: true)
    container.addSubview(circle)

    let sideWidth = (popupWidth - circleSize - margins * 2) / 2
    level = MetricPieChartView(
      frame: NSRect(x: (sideWidth - 60) / 2, y: 10, width: 60, height: 50),
      segments: [
  ...
  private func initChart() -> NSView {
    ...
    lineChart = MetricLineChartView(
      frame: NSRect(x: 1, y: 0, width: popupWidth - 2, height: chartH), num: initialChartSamples,
      fixedMax: 1)
  ...
  private func initDetails() -> NSView {
    ...
    compField =
      addPopupColorValueRow(container, color: compressedColor, title: "Compressed:", value: "").2
    freeField =
      addPopupColorValueRow(
        container, color: freeColor.withAlphaComponent(0.5), title: "Free:", value: ""
      ).2
  ...
  func updateRAMUsage(
    _ detail: RAMUsageDetail, processes: [RankedProcessMetric], syncHistory _: Bool = false
  ) {
    ...
    circle.setSegmentValues([
      ChartSegmentValue(
        detail.totalBytes > 0 ? Double(detail.appBytes) / Double(detail.totalBytes) : 0,
        color: appColor),
      ChartSegmentValue(
        detail.totalBytes > 0 ? Double(detail.wiredBytes) / Double(detail.totalBytes) : 0,
        color: wiredColor),
      ChartSegmentValue(
        detail.totalBytes > 0 ? Double(detail.compressedBytes) / Double(detail.totalBytes) : 0,
...
final class GPUPopupView: NSStackView {
  ...
  private func makeStatsView(showRenderTiler: Bool) -> NSView {
    ...
    let labelRow = NSStackView(
      frame: NSRect(x: 0, y: chartRowH + circleRowH, width: popupWidth, height: labelH))
  ...
  private func makeCircle(id: String) -> MetricPieChartView {
    let c = MetricPieChartView(
      frame: NSRect(x: 0, y: 0, width: circleSize, height: circleSize), openCircle: true)
    c.id = id
    return c
  }

  private func makeChart(id: String) -> MetricLineChartView {
    let c = MetricLineChartView(
      frame: NSRect(x: 0, y: 0, width: 100, height: chartSize), num: initialChartSamples,
      fixedMax: 1)
    c.id = id
    c.wantsLayer = true
...
final class PowerPopupView: NSStackView {
  ...
  private func initRows() {
    ...
    (_, _, memoryValue) = addPopupColorValueRow(
      self, color: memoryColor, title: "Memory", value: "--W")
    (_, _, mediaValue) = addPopupColorValueRow(
      self, color: mediaColor, title: "Media", value: "--W")
    (_, _, displayValue) = addPopupColorValueRow(
      self, color: displayColor, title: "Display", value: "--W")
    (_, _, otherValue) = addPopupColorValueRow(
      self, color: otherColor, title: "Other SoC", value: "--W")
  }
  ...
  private func initFlow() -> NSView {
    ...
    flowView = BatteryPowerFlowView(
      frame: NSRect(x: margins, y: 0, width: popupWidth - margins * 2, height: flowH))
  ...
  private func initChart() -> NSView {
    ...
    chart = PowerComponentStackedChartView(
      frame: NSRect(
        x: margins, y: margins, width: popupWidth - margins * 2, height: chartH - margins * 2),
      cpuColor: cpuColor,
      gpuColor: gpuColor,
      aneColor: aneColor,
  ...
  private func fmtPower(_ watts: Double?) -> String {
    ...
    switch watts {
    case ..<10:
      return MactopFormat.string("%.1f W", watts)
    default:
      return MactopFormat.string("%.0f W", watts)
    }
  }
}
...
final class NetworkPopupView: NSStackView {
  ...
  private func topValueView(_ container: NSView, title: String, color: NSColor) -> (
    NSView, NSTextField, NSTextField
  ) {
    let topH: CGFloat = 30
    ...
    unitField.isBordered = false
    unitField.drawsBackground = false

    let titleWidth =
      title.widthOfString(usingFont: NSFont.systemFont(ofSize: 12, weight: .regular)) + 8
  ...
  private func initDetails() -> NSView {
    ...
    let (_, _, tu) = addPopupColorValueRow(
      view, color: uploadColor, title: "Total upload:", value: "0")
    let (_, _, td) = addPopupColorValueRow(
      view, color: downloadColor, title: "Total download:", value: "0")
    totalUpField = tu
    totalDownField = td
  ...
  private func setSpeedFields(upload: Double, download: Double) {
    ...
    if let downloadView, let downloadValueField, let downloadUnitField {
      ...
      downloadView.setFrameOrigin(
        NSPoint(x: (halfW - downTopW) / 2, y: downloadView.frame.origin.y))
